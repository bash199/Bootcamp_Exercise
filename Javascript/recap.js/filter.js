...
const rating = (array,rate)=>{
   let result = array.filter(element =>{
      return element.rating.average > rate ;
   });
   return result;
};
rating(movies,5)
...
const ratingAndRuntime = (array,rate,time)=>{
   let result2 = array.filter(element => {
      return ( element.rating.average > rate) && (element.runtime = time);
   });
   return result2;
};
ratingAndRuntime(movies,8,50)
...
const ratingAndRuntime2 = (array,rate,time)=>{
   let resultFilter = array.filter(element => {
      return (element.rating.average > rate) && (element.runtime = time)  
   })
   let resultMap =resultFilter.map(({name,rating})=>{
      return {name:name,rating:rating};
   })
   return resultMap;
};
console.log(ratingAndRuntime2(movies,5,50));
